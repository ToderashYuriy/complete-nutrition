...
var bcSfFilterTemplate = {
    ...
    'soldOutLabelHtml': '<div class="product__label label"><p class="product__label-text label__text">' + bcSfFilterConfig.label.sold_out + '</p></div>',
    'saleLabelHtml': '<div class="product__label label"><p class="product__label-text label__text">' + bcSfFilterConfig.label.on_sale + '</p></div>',
    'vendorHtml': '<h4 class="product__vendor h6">{{itemVendorLabel}}</h4>',

    // Grid Template
    'productGridItemHtml': '<div class="inner_content_block_filter o-layout__item ' + bcSfFilterConfig.custom.grid_item_width + '">' +
                            '<div class="product product--sale product--natural product--center">{{itemIconStatus}}' +
                                '<div class="product-top">' +
                                    '<a href="{{itemUrl}}" class="product-link" title="{{itemTitle}}">' + 
                                        ...
                                        '</div>' +
                                    '</a>' +
                                    '<div class="product_quick_hidden"><div class="background_hover_btns"></div>' +
                                        '<div class="product_hover_btns">' +
                                        '<a href="{{itemUrl}}" class="viev_details">VIEW DETAILS</a>' +
                                        '<a href="{{itemUrl}}" class="quick_shop_btn js-no-transition">QUICK SHOP</a>' +
                                        '</div>' +
                                    '</div>' +
                                '</div>' +
                                '<a href="{{itemUrl}}" class="product-link" title="{{itemTitle}}">' + 
...
BCSfFilter.prototype.buildProductGridItem = function(data) {
    ...
    var imageSrc = this.getFeaturedImage(images, '1x1').replace('_1x1.', '_{width}x.');
    var itemThumbnailHtml = '<img  src="' + this.getFeaturedImage(images, bcSfFilterConfig.custom.image_size) + '"' +
                            'alt="{{itemTitle}}" ' +
                            'data-src="' + imageSrc + '" ' +
                            'class="product__img lazyload" ' +
                            'data-widths="[180, 360, 540, 720, 900, 1080, 1296, 1512]" ' +
                            'data-sizes="auto" />';
    itemHtml = itemHtml.replace(/{{itemThumbnail}}/g, itemThumbnailHtml);
    ...
    if (!soldOut) {
        if (onSale) {
            if (priceVaries) {
                itemPriceHtml += '<span class="product__price-price product__price-price--sale"><span class="money">' + this.formatMoney(data.price_min) +'</span></span>';
    ...
    var statusInner = '';
    if(tags.indexOf('Tag-CustomerFavorite') > -1){
    statusInner =   '<div class="product__label label label--brand label-yellow">'+
                        '<p class="product__label-text label__text">Customer Favorite</p>' +
                    '</div>';
    }else if(tags.indexOf('Tag-New') > -1){
    statusInner =   '<div class="product__label label label--brand label--red">' +
                        '<p class="product__label-text label__text">New</p>' +
                    '</div>';
    }else if(tags.indexOf('Tag-BestSeller') > -1){
    statusInner =   '<div class="product__label label label--brand label-yellow">' +
                        '<p class="product__label-text label__text">BEST SELLER</p>' +
                    '</div>';
    }else if(data.price_max < data.compare_at_price){
    statusInner =   '<div class="product__label label label--brand label--red">' +
                        '<p class="product__label-text label__text">Sale</p>' +
                    '</div>';
    }
    itemHtml = itemHtml.replace(/{{itemIconStatus}}/g, statusInner);
...
var bcSfFilterTemplate = {
    ...
    'productGridItemHtml': '<div class="o-layout__item ' + bcSfFilterConfig.custom.grid_item_width + '">' +
                                '<a href="{{itemUrl}}" class="product-link" title="{{itemTitle}}">' + 
                                    '<div class="product {{soldOutClass}} product--' + bcSfFilterConfig.custom.product_grid_align + '">' +        
